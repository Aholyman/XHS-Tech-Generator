...
export async function generatePostData(topic: string): Promise<PostData> {
  ...
  const response = await ai.models.generateContent({
    ...
    CRITICAL: Search for the LATEST news and insights about: ${topic} from the LAST 3 DAYS. Do NOT use outdated information. You MUST use the googleSearch tool to verify the latest facts.
    
    Write a Xiaohongshu (Little Red Book) style tech post IN CHINESE.
    Requirements:
    1. Title: Attractive, high information density, not clickbait.
    2. Content: Conversational, easy to understand, high information density, use emojis appropriately.
    3. Cards: Generate EXACTLY 4 image cards for this post.
       ...
       - CRITICAL CHARACTER LIMITS:
         * For 'image_top' or 'image_bottom' cards: STRICTLY 100-140 characters. (Do not exceed, or it will overflow).
         * For 'text_only' cards: STRICTLY 250-350 characters. (Provide deep, detailed insights, multiple bullet points, to ensure high information density and fill the empty space).
